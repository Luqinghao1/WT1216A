...
// ===========================================================================
// FittingDataLoadDialog 实现 (数据加载对话框)
// ===========================================================================
FittingDataLoadDialog::FittingDataLoadDialog(const QList<QStringList>& previewData, QWidget *parent) : QDialog(parent) {
    ...
    // 强制黑色字体
    setStyleSheet("QDialog { background-color: #f0f0f0; color: black; } QLabel, QComboBox, QPushButton, QTableWidget, QGroupBox { color: black; }");
...
FittingWidget::FittingWidget(QWidget *parent) :
    ...
    m_plotTitle(nullptr),
    m_isFitting(false),
    m_currentModelType(ModelManager::Model_1) // 默认初始模型为 Model 1
{
    ui->setupUi(this);
    ...
    // 注意：ui->btn_modelSelect 的点击信号已通过 on_btn_modelSelect_clicked 自动连接，无需在此手动 connect
...
void FittingWidget::on_btn_modelSelect_clicked() {
    ...
    if (dlg.exec() == QDialog::Accepted) {
        ...
        if (found) {
            ...
            on_btnResetParams_clicked(); // 切换模型后重置参数
        } else {
...
void FittingWidget::on_btnResetParams_clicked() {
    ...
    if(!defs.contains("gamaD")) defs["gamaD"] = 0.02; // 默认压敏系数
    ...
    for(const QString& key : orderedKeys) {
        if(defs.contains(key)) {
            ...
            else if (key == "gamaD") { p.min = 0.0; p.max = 1.0; } // 压敏系数范围
...
void FittingWidget::on_btnExportData_clicked() {
    updateParamsFromTable();
    QString fileName = QFileDialog::getSaveFileName(this, "导出拟合参数", "FittingParameters.csv", "CSV Files (*.csv);;Text Files (*.txt)");
...
void FittingWidget::on_btnExportChart_clicked() {
    QString fileName = QFileDialog::getSaveFileName(this, "导出图表", "FittingChart.png", "PNG Image (*.png);;JPEG Image (*.jpg);;PDF Document (*.pdf)");
    if (fileName.isEmpty()) return;
    bool success = false;
